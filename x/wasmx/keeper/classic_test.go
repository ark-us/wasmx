...
// 	codeId, contractAddress := appA.DeployEvm(sender, evmcode, types.WasmxExecutionMessage{Data: []byte{}}, nil)
// 	// contractAddress := appA.InstantiateCode(sender, codeId, types.WasmxExecutionMessage{Data: []byte{}}, "opcodetest", nil)

// 	fmt.Println("---deployed", codeId, contractAddress.String())
...
func (suite *KeeperTestSuite) TestEwasmSimpleStorage() {
	...
	_, contractAddress := appA.DeployEvm(sender, evmcode, types.WasmxExecutionMessage{Data: []byte{}}, nil, "simpleStorage")
...
// 	codeIdFibo := appA.StoreCodeInterpreted(sender, fiboevm)
...
// 	codeId1 := appA.StoreCodeInterpreted(sender, evmcode)
// 	contractAccount1 := appA.InstantiateCode(sender, codeId1, types.WasmxExecutionMessage{Data: []byte{}}, "callgeneralwasm1", nil)

// 	// Contract 2
// 	contractAccount2 := appA.InstantiateCode(sender, codeId1, types.WasmxExecutionMessage{Data: []byte{}}, "callgeneralwasm2", nil)

// 	// Contract 3
// 	contractAccount3 := appA.InstantiateCode(sender, codeId1, types.WasmxExecutionMessage{Data: []byte{}}, "callgeneralwasm3", nil)
...
// 	codeId1 := appA.StoreCodeInterpreted(sender, evm_inner)
...
// 	codeId2 := appA.StoreCodeInterpreted(sender, evmbin_deep)
...
// 	codeId := appA.StoreCodeInterpreted(sender, evmbin)
...
// 	codeId := appA.StoreCodeInterpreted(sender, evmcode_inner)
...
// 	codeId2 := appA.StoreCodeInterpreted(sender, evmcode)
...
// 	// Deploy library code
// 	codeIdLib := appA.StoreCodeInterpreted(sender, evmcodelib)
...
// 	sender := suite.GetRandomAccount()
// 	initBalance := sdk.NewInt(1000_000_000)
// 	evmToCreate, err := hex.DecodeString(testdata.CallStaticInner)
// 	s.Require().NoError(err)
// 	evmcode, err := hex.DecodeString(testdata.Create2)
// 	s.Require().NoError(err)
...
// 	codeId := appA.StoreCodeInterpreted(sender, evmToCreate)
...
// 	codeId := appA.StoreCodeInterpreted(sender, originbz)
...
// 	s.Require().NoError(err)
// 	codeIdCall := appA.StoreCodeInterpreted(sender, evmcodeCall)
