...
func (suite *KeeperTestSuite) TestEwasmPrecompileCurve384Direct2() {
	...
	// test_cmul
	// suite.ExecuteContractWithOpts(sender, contractAddress, fmt.Sprintf(`{"readonly": true, "data": "0x%s"}`, "e5582b4d"), nil, 1_000_000_000_000, deps)
...
// 	// qres := appA.EwasmQuery(sender, contractAddress, types.WasmxExecutionMessage{Data: suite.hex2bz(fmt.Sprintf("0x%s%s%s%s%s%s"}`, verifySignatureFast, msgHash, rhi, rlo, shi, slo)), nil)
