...
func (k Keeper) execute(ctx sdk.Context, contractAddress sdk.AccAddress, caller sdk.AccAddress, msg []byte, coins sdk.Coins, dependencies []string) ([]byte, error) {
	...
	// prepare querier
	handler := k.newCosmosHandler(ctx, contractAddress)
	res, gasUsed, execErr := k.wasmvm.Execute(ctx, codeInfo.CodeHash, codeInfo.Pinned, env, info, msg, prefixStoreKey, k.ContractStore(ctx, prefixStoreKey), handler, k.gasMeter(ctx), codeInfo.Deps, deps)
	k.consumeRuntimeGas(ctx, gasUsed)
...
func (k Keeper) query(ctx sdk.Context, contractAddress sdk.AccAddress, caller sdk.AccAddress, msg []byte, coins sdk.Coins, dependencies []string) ([]byte, error) {
	...
	handler := k.newCosmosHandler(ctx, contractAddress)
	res, gasUsed, execErr := k.wasmvm.QueryExecute(ctx, codeInfo.CodeHash, codeInfo.Pinned, env, info, msg, prefixStoreKey, k.ContractStore(ctx, prefixStoreKey), handler, k.gasMeter(ctx), dependencies, deps)
	k.consumeRuntimeGas(ctx, gasUsed)
