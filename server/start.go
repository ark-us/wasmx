...
import (
	...
	srvflags "wasmx/server/flags"
	websrv "wasmx/x/websrv/server"
...
`,
	...
	cmd.Flags().String(srvflags.GRPCWebAddress, sdkserverconfig.DefaultGRPCWebAddress, "The gRPC-Web server address to listen on")
...
func startInProcess(ctx *server.Context, clientCtx client.Context, appCreator types.AppCreator) (err error) {
	...
	if config.GRPC.Enable {
		grpcSrv, err = sdkgrpc.StartGRPCServer(clientCtx, app, config.GRPC)
		if err != nil {
			return err
		}
		defer grpcSrv.Stop()
		if config.GRPCWeb.Enable {
			grpcWebSrv, err = sdkgrpc.StartGRPCWeb(grpcSrv, config.Config)
			if err != nil {
	...
	if config.Websrv.Enable {
		...
		httpSrv, httpSrvDone, err = websrv.StartWebsrv(ctx, clientCtx, &config.Websrv)
		if err != nil {
			return err
		}
		defer func() {
