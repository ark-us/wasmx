...
do
    ...
    if [[ $label = 'erc20-rollup' ]]; then
        ...
        mv -f $WASMX_ERC20_ROLLUP/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/4c.erc20rollupjson_0.0.1.wasm
    fi
    if [[ $label = 'gov' ]]; then
        echo "building gov"
    ...
    if [[ $label = 'distribution' ]]; then
        ...
        mv -f $WASMX_DISTRIBUTION/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/46.distribution_0.0.1.wasm
    fi
    if [[ $label = 'chat' ]]; then
        echo "building chat"
        cd $WASMX_CHAT && npm run asbuild
        mv -f $WASMX_CHAT/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/42.chat_0.0.1.wasm
    fi
    if [[ $label = 'chatverif' ]]; then
        echo "building chat verifier"
        cd $WASMX_CHAT_VERIFIER && npm run asbuild
        mv -f $WASMX_CHAT_VERIFIER/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/44.chat_verifier_0.0.1.wasm
    fi
    if [[ $label = 'time' ]]; then
        echo "building time"
        cd $WASMX_TIME && npm run asbuild
        mv -f $WASMX_TIME/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/47.time_0.0.1.wasm
    fi
    if [[ $label = 'level0' ]]; then
        echo "building level0"
        cd $WASMX_LEVEL0 && npm run asbuild
        mv -f $WASMX_LEVEL0/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/48.level0_0.0.1.wasm
    fi
    ...
    if [[ $label = 'multichain' ]]; then
        ...
        mv -f $WASMX_MULTICHAIN/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/4a.multichain_registry_0.0.1.wasm
    fi
    if [[ $label = 'multichain_local' ]]; then
        echo "building multichain_local"
        cd $WASMX_MULTICHAIN_LOCAL && npm run asbuild
        mv -f $WASMX_MULTICHAIN_LOCAL/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/4b.multichain_registry_local_0.0.1.wasm
    fi
    if [[ $label = 'lobby' ]]; then
        echo "building lobby"
        cd $WASMX_LOBBY && npm run asbuild
        mv -f $WASMX_LOBBY/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/4d.lobby_json_0.0.1.wasm
    fi
    if [[ $label = 'metaregistry' ]]; then
        echo "building metaregistry"
        cd $WASMX_METAREGISTRY && npm run asbuild
        mv -f $WASMX_METAREGISTRY/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/4f.metaregistry_json_0.0.1.wasm
    fi
    # if [[ $label = 'params' ]]; then
    #     echo "building params"
    #     cd $WASMX_PARAMS && npm run asbuild
    #     mv -f $WASMX_PARAMS/build/$BINARY_TYPE.wasm $WASMX_GO_PRECOMPILES/50.params_json_0.0.1.wasm
    # fi
    if [[ $label = 'codes' ]]; then
        echo "building codes registry"
    ...
    if [[ $label = 'tests' ]]; then
        ...
        mv -f $WASMX_TESTS_CROSSCHAIN/build/$BINARY_TYPE.wasm $WASMX_GO_TESTDATA_NETWORK/crosschain.wasm
        ...
        mv -f $WASMX_TESTS_SMTP/build/$BINARY_TYPE.wasm $WASMX_GO_TESTDATA_SMTP/wasmx_test_smtp.wasm

        cd $WASMX_TESTS_HTTP && npm run asbuild
        mv -f $WASMX_TESTS_HTTP/build/$BINARY_TYPE.wasm $WASMX_GO_TESTDATA_HTTP/wasmx_test_http.wasm
    fi
    if [[ $label = 'imap' ]]; then
        cd $WASMX_TESTS_IMAP && npm run asbuild
        mv -f $WASMX_TESTS_IMAP/build/$BINARY_TYPE.wasm $WASMX_GO_TESTDATA_IMAP/wasmx_test_imap.wasm
