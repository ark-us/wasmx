...
func (k *Keeper) PeekAutoIncrementID(ctx sdk.Context) uint64 {
	store := ctx.KVStore(k.storeKey)
	key := types.GetLastCodeIDKey()
	bz := store.Get(key)
	id := uint64(1)
...
func (k *Keeper) importAutoIncrementID(ctx sdk.Context, val uint64) error {
	store := ctx.KVStore(k.storeKey)
	key := types.GetLastCodeIDKey()
	if store.Has(key) {
