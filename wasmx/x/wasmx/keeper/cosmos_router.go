...
func (h *WasmxCosmosHandler) Deploy(bytecode []byte, sender *mcodec.AccAddressPrefixed, provenance *mcodec.AccAddressPrefixed, initMsg []byte, value *big.Int, deps []string, metadata types.CodeMetadata, label string, salt []byte, source []byte) (codeId uint64, checksum []byte, contractAddress mcodec.AccAddressPrefixed, err error) {
	funds := sdk.NewCoins(sdk.NewCoin(h.Keeper.denom, sdkmath.NewIntFromBigInt(value)))
	return h.Keeper.CreateInterpreted(h.Ctx, sender, provenance, bytecode, deps, metadata, initMsg, funds, label, salt, source)
}
...
func (k *Keeper) ExecuteCosmosMsg(ctx sdk.Context, msg sdk.Msg, owner mcodec.AccAddressPrefixed) ([]sdk.Event, []byte, error) {
	...
	if err != nil {
		return nil, nil, sdkerrors.ErrUnauthorized.Wrapf("wasmx cosmos message signer from %s: %s", sdk.MsgTypeURL(msg), err.Error())
	}
	if len(signers) == 0 {
		return nil, nil, sdkerrors.ErrUnauthorized.Wrapf("wasmx cosmos message signer missing for %s", sdk.MsgTypeURL(msg))
	}
	authorized := bytes.Equal(signers[0], owner.Bytes())
	signerstr, err := k.AddressCodec().BytesToString(sdk.AccAddress(signers[0]))
	if err != nil {
		return nil, nil, sdkerr.Wrapf(err, "signer: %s for %s", mcfg.ERRORMSG_ACC_TOSTRING, sdk.MsgTypeURL(msg))
	}
