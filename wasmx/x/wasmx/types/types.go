...
func NewCodeInfo(codeHash []byte, creatorBech32 string, deps []string, metadata CodeMetadata, pinned bool, meteringOff bool, source []byte) CodeInfo {
	return CodeInfo{
		CodeHash:       codeHash,
...
func NewInfo(gasMeter MultipliedGasMeter, origin mcodec.AccAddressPrefixed, creator mcodec.AccAddressPrefixed, deposit sdk.Coins) MessageInfo {
	funds := big.NewInt(0)
	if len(deposit) > 0 {
	...
	return MessageInfo{
		...
		Funds:    funds,
		Origin:   origin,
		GasLimit: big.NewInt(int64(gasMeter.GasLimit())),
	}
}
