...
func (b *Backend) BlockNumber() (hexutil.Uint64, error) {
	...
	_, err := b.queryClient.Params(b.ctx, &wasmxtypes.QueryParamsRequest{}, grpc.Header(&header))
	if err != nil {
...
) ([]*ethtypes.Transaction, []common.Hash) {
...
func (b *Backend) BlockBloom(blockRes *tmrpctypes.ResultBlockResults) (ethtypes.Bloom, error) {
	...
	// 	if event.Type != wasmxtypes.EventTypeBlockBloom {
	// 		continue
	// 	}
...
func (b *Backend) RPCBlockFromTendermintBlock(
	...
	ethTxs, txHashes := b.EthMsgsFromTendermintBlock(resBlock, blockRes)
	for txIndex, tx := range ethTxs {
		if !fullTx {
			// hash := common.HexToHash(ethMsg.Hash)
			ethRPCTxs = append(ethRPCTxs, txHashes[txIndex])
			continue
		}
