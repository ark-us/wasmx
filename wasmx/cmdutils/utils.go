...
// to Viper.
func InterceptConfigsAndCreateContext(cmd *cobra.Command, customAppConfigTemplate string, customAppConfig interface{}, cmtConfig *tmcfg.Config) (*sdkserver.Context, error) {
	serverCtx := sdkserver.NewDefaultContext()

	// backup for mobile ios/android builds
	nodeDaemonHome, _ := cmd.Flags().GetString(flagNodeDaemonHome)
	basename := nodeDaemonHome
...
// viperCfg object.
func interceptConfigs(rootViper *viper.Viper, customAppTemplate string, customConfig interface{}, cmtConfig *tmcfg.Config) (*tmcfg.Config, error) {
	...
	case os.IsNotExist(err):
		...
		tmcfg.WriteConfigFile(cmtCfgFile, conf)
	...
	if _, err := os.Stat(appCfgFilePath); os.IsNotExist(err) {
		if customAppTemplate != "" {
			srvcfg.SetConfigTemplate(customAppTemplate)

			if err = rootViper.Unmarshal(&customConfig); err != nil {
				return nil, fmt.Errorf("failed to parse %s: %w", appCfgFilePath, err)
			}

			srvcfg.WriteConfigFile(appCfgFilePath, customConfig)
		} else {
			appConf, err := srvcfg.ParseConfig(rootViper)
			if err != nil {
				return nil, fmt.Errorf("failed to parse %s: %w", appCfgFilePath, err)
			}

			srvcfg.WriteConfigFile(appCfgFilePath, appConf)
		}
	}
