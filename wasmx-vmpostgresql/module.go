...
func (am AppModule) EndTransaction(ctx context.Context, txmode sdk.ExecMode, gInfo sdk.GasInfo, result *sdk.Result, anteEvents []abci.Event, txerr error) error {
	...
	for _, conn := range conns {
		...
		if shouldCommit {
			...
			_, err := conn.OpenSavepointTx.Tx().Exec(ctx, cmd)
			if err != nil {
				return fmt.Errorf("db tx command failed: %s, %s", cmd, err.Error())
			}
			err = conn.OpenSavepointTx.Tx().Commit(ctx)
			if err != nil {
				return fmt.Errorf("cannot commit postgresql open tx: %s", err.Error())
			}
		} else {
			err = conn.OpenSavepointTx.Tx().Rollback(ctx)
			if err != nil {
...
func (am AppModule) EndSubCall(ctx context.Context, level uint32, index uint32, isquery bool, txerr error) error {
