...
func (suite *KeeperTestSuite) TestWasiInterpreterPythonBlockchain() {
	...
	expectedContractAddress := appA.AccBech32Codec().BytesToAccAddressPrefixed(sdk.AccAddress(vmtypes.CleanupAddress(resp)))
	contractInfo, err := appA.App.WasmxKeeper.GetContractInfo(appA.Context(), expectedContractAddress)
	s.Require().NoError(err)
	...
	createdContractAddress, err := appA.AccBech32Codec().StringToAccAddressPrefixed(createdContractAddressStr)
	s.Require().NoError(err)
