...
func GetActiveValidatorInfo(validators []staking.Validator) ([]consensus.TendermintValidator, error) {
    ...
    for _, v := range validators {
        if IsValidatorInactive(v) { continue }
        ...
        out = append(out, consensus.TendermintValidator{
            ...
            HexAddress:       wasmx.HexString(addrhex),
...
func FilterAndSortCommitSignatures(sigs []consensus.CommitSig, infos []consensus.TendermintValidator) []consensus.CommitSig {
    ...
    for _, v := range infos { active[string(v.HexAddress)] = struct{}{} }
    out := make([]consensus.CommitSig, 0, len(sigs))
    for _, s := range sigs {
        if _, ok := active[string(s.ValidatorAddress)]; ok { out = append(out, s) }
    }
    return out
}
...
func GetTxsHash(txs [][]byte) []byte {
    ...
    for i := range txs { slices[i] = base64.StdEncoding.EncodeToString(txs[i]) }
...
func ExtractIndexedTopics(finalizeResp consensus.ResponseFinalizeBlock, txhashes [][]byte) []IndexedTopic {
    ...
    push := func(topic string, txhash []byte) {
        if len(topic) > MaxKeyLength { return }
        arr := topicMap[topic]
    ...
    for k, v := range topicMap { out = append(out, IndexedTopic{Topic: k, Values: v}) }
    return out
}

// Simple error helper
func wasmxError(msg string) error { wasmx.LoggerError("consensus-utils", msg, nil); return &utilsError{msg} }
