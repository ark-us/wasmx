...
func SubmitProposal(req MsgSubmitProposal) []byte {
	...
	groupDenom := params.MinDeposit[0].Denom
	if groupPower.Denom != "" {
		groupDenom = groupPower.Denom
	}
	if len(deposit) == 0 {
		amount := NewSdkIntZero()
		deposit = []wasmx.Coin{{Denom: groupDenom, Amount: amount}}
	}
	if deposit[0].Denom != groupDenom {
		Revert("invalid denom; expected " + groupDenom + ", got " + deposit[0].Denom)
	}
...
func DoDeposit(req MsgDeposit) []byte {
	...
	if proposal.GroupDenom != "" {
		for _, c := range req.Amount {
			if c.Denom != proposal.GroupDenom {
				Revert("invalid denom; expected " + proposal.GroupDenom + ", got " + c.Denom)
			}
		}
	}
