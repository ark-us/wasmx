...
func SignMessage(msg string) (string, error) {
    ...
    if len(st.ValidatorPrivkey) == 0 { return "", errors.New("empty validator privkey") }
...
func getNodeByAddress(addr string, nodes []NodeInfo) (*NodeInfo, int) {
    for i := range nodes { if nodes[i].Address == addr { return &nodes[i], i } }
    return nil, -1
}
...
func verifyMessage(nodeIndex int32, signatureStr string, msg string) (bool, error) {
    ...
    if int(nodeIndex) < 0 || int(nodeIndex) >= len(nodes) { return false, nil }
...
func verifyMessageByAddr(addr string, signatureStr string, msg string) (bool, error) {
    ...
    if err != nil { return false, err }
    if pubKey == nil { return false, nil }
...
func setFinalizedBlock(blockData []byte, hash string, txhashes [][]byte, indexedTopics []blocks.IndexedTopic) error {
    ...
    for i := range txhashes { payload.Txhashes[i] = base64.StdEncoding.EncodeToString(txhashes[i]) }
    calld := blocks.CallData{SetBlock: &payload}
    bz, err := json.Marshal(&calld)
    if err != nil { return err }
    resp, err := callStorage(string(bz), false)
    if err != nil { return err }
    if resp.Success > 0 { return errors.New("could not set finalized block: "+resp.Data) }
    return nil
}
...
func getLastBlockIndex() (int64, error) {
    ...
    if err != nil { return 0, err }
    if resp.Success > 0 { return 0, errors.New("could not get last block index") }
    var res blocks.LastBlockIndexResult
    if err := json.Unmarshal([]byte(resp.Data), &res); err != nil { return 0, err }
...
func getFinalBlock(index int64) (string, error) {
    ...
    resp, err := callStorage(string(bz), true)
    if err != nil { return "", err }
    if resp.Success > 0 { return "", fmt.Errorf("could not get finalized block: %d", index) }
    return resp.Data, nil
}

func setConsensusParams(height int64, value *consensus.ConsensusParams) error {
    var params []byte
    if value != nil {
    ...
    resp, err := callStorage(string(bz), false)
    if err != nil { return err }
    if resp.Success > 0 { return errors.New("could not set consensus params: "+resp.Data) }
    return nil
}

func getConsensusParams(height int64) (*consensus.ConsensusParams, error) {
    calld := blocks.CallData{GetConsensusParams: &blocks.CallDataGetConsensusParams{Height: height}}
    bz, err := json.Marshal(&calld)
    if err != nil { return nil, err }
    resp, err := callStorage(string(bz), true)
    if err != nil { return nil, err }
    if resp.Success > 0 { return nil, errors.New("could not get consensus params: "+resp.Data) }
    if resp.Data == "" { return nil, nil }
    var params consensus.ConsensusParams
    if err := json.Unmarshal([]byte(resp.Data), &params); err != nil { return nil, err }
    return &params, nil
}

// UpdateConsensusParams merges updates into existing params and stores them for height+1
func updateConsensusParams(height int64, updates *consensus.ConsensusParams) error {
    if updates == nil {
        return setConsensusParams(height+1, nil)
    }
    params, err := getConsensusParams(height)
    if err != nil { return err }
    if params == nil { params = &consensus.ConsensusParams{} }
    ...
    if updates.Block.MaxBytes != 0 { params.Block.MaxBytes = updates.Block.MaxBytes }
    if updates.Block.MaxGas != 0 { params.Block.MaxGas = updates.Block.MaxGas }
    if updates.Evidence.MaxAgeDuration != 0 { params.Evidence.MaxAgeDuration = updates.Evidence.MaxAgeDuration }
    if updates.Evidence.MaxAgeNumBlocks != 0 { params.Evidence.MaxAgeNumBlocks = updates.Evidence.MaxAgeNumBlocks }
    if updates.Evidence.MaxBytes != 0 { params.Evidence.MaxBytes = updates.Evidence.MaxBytes }
    if len(updates.Validator.PubKeyTypes) > 0 { params.Validator.PubKeyTypes = updates.Validator.PubKeyTypes }
    if updates.Version.App != 0 { params.Version.App = updates.Version.App }
    return setConsensusParams(height+1, params)
}

// Expose select consensus-utils helpers
func extractIndexedTopics(resp consensus.ResponseFinalizeBlock, txhashes [][]byte) []blocks.IndexedTopic {
    topics := consutils.ExtractIndexedTopics(resp, txhashes)
    out := make([]blocks.IndexedTopic, len(topics))
    for i := range topics { out[i] = blocks.IndexedTopic{Topic: topics[i].Topic, Values: topics[i].Values} }
    return out
}

// verifyBlockProposalMeta verifies header hash equals request hash when header is available in metainfo
func verifyBlockProposalMeta(wrap consensus.RequestProcessProposalWithMetaInfo) error {
    if wrap.Metainfo == nil { return nil }
    hbz, ok := wrap.Metainfo["header"]
    if !ok || len(hbz) == 0 { return nil }
    var header consensus.Header
    if err := json.Unmarshal(hbz, &header); err != nil { return err }
    hhash, err := consensus.HeaderHash(header)
    if err != nil { return err }
    if len(hhash) == 0 || len(wrap.Request.Hash) == 0 { return nil }
...
func verifyBlockProposal(wrap consensus.RequestProcessProposalWithMetaInfo, fin *consensus.ResponseFinalizeBlock) error {
    if wrap.Metainfo == nil { return nil }
    hbz, ok := wrap.Metainfo["header"]
    if !ok || len(hbz) == 0 { return nil }
    var header consensus.Header
    ...
    if cbz, ok := wrap.Metainfo["commit"]; ok && len(cbz) > 0 {
        var commit consensus.BlockCommit
    ...
    if vals, err := buildActiveTendermintValidators(); err == nil && len(vals) > 0 {
        vh, err := consensus.ValidatorsHash(vals)
        if err == nil && !hexEqual(string(header.ValidatorsHash), vh) {
...
func hexEqual(hexStr string, bz []byte) bool {
    if len(bz) == 0 { return hexStr == "" }
    enc := hex.EncodeToString(bz)
    return strings.EqualFold(hexStr, enc)
}
// Tendermint helpers
func getBlockID(hash []byte) consensus.BlockID {
    hexhash := strings.ToUpper(hex.EncodeToString(hash))
    return consensus.BlockID{Hash: wasmx.HexString(hexhash), Parts: consensus.PartSetHeader{Total: 1, Hash: wasmx.HexString(hexhash)}}
}
...
func buildBlockEntry(height int64, wrap consensus.RequestProcessProposalWithMetaInfo, finResp *consensus.ResponseFinalizeBlock, proposer string, validators *consensus.TendermintValidators) ([]byte, error) {
    // header and commit may be present in metainfo
    var header []byte
    var commit []byte
    if wrap.Metainfo != nil {
        if v, ok := wrap.Metainfo["header"]; ok { header = v }
        if v, ok := wrap.Metainfo["commit"]; ok { commit = v }
    }
    // validator set
    var valbz []byte
    if validators != nil {
...
func doOptimisticExecution(processReq consensus.RequestProcessProposal, processResp consensus.ResponseProcessProposal) (consensus.ResponseOptimisticExecution, error) {
    return consensus.OptimisticExecution(processReq, processResp)
}
...
func getAllValidators() ([]stakinglib.Validator, error) {
    ...
    if err != nil { return nil, err }
    if resp.Success > 0 || resp.Data == "" { return []stakinglib.Validator{}, nil }
    var out struct{ Validators []stakinglib.Validator `json:"validators"` }
    if err := json.Unmarshal([]byte(resp.Data), &out); err != nil { return []stakinglib.Validator{}, nil }
    return out.Validators, nil
}

func getCurrentValidator() consensus.ValidatorInfo {
    st, _ := GetCurrentState()
    return consensus.ValidatorInfo{Address: wasmx.HexString(st.ValidatorAddress), PubKey: st.ValidatorPubkey, VotingPower: 0, ProposerPriority: 0}
}

func checkValidatorsUpdate(validators []consensus.ValidatorInfo, validatorInfo consensus.ValidatorInfo, nodeId int32) error {
    if int(nodeId) >= len(validators) { return errors.New("validator index out of range") }
...
func initializeIndexArrays(lenNodes int) error {
    ...
    if err := SetNextIndexArray(nextIndex); err != nil { return err }
