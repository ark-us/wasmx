...
func GetMempool() (Mempool, error) {
	...
	if mp.Map == nil {
		mp.Map = map[string]MempoolTx{}
	}
	if mp.Temp == nil {
		mp.Temp = map[string]bool{}
	}
	if mp.ToBeSent == nil {
		mp.ToBeSent = map[string]bool{}
	}
	if mp.Order == nil {
		mp.Order = []string{}
	}
...
func AppendLogEntry(entry LogEntryAggregate) error {
	...
	if err := SetLastLogIndex(index); err != nil {
		return err
	}
...
func SetLogEntry(index int64, entry string) error {
	...
	if key == "" {
		return errors.New("empty key for log entry")
	}
	sSet(key, entry)
	return nil
}
...
func GetLogEntryObj(index int64) (LogEntry, error) {
	...
	if v == "" {
		return LogEntry{Index: 0, TermID: 0, LeaderID: 0, Data: []byte{}}, nil
	}
	var e LogEntry
