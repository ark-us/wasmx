...
import (
	...
	kayrosp2p "github.com/loredanacirstea/wasmx-kayrosp2p-lib/lib"
	lib "github.com/loredanacirstea/wasmx-kayrosp2p-ondemand-lib/lib"
...
func main() {
	...
	case "ifNewTransaction":
		...
		wasmx.Finish(res)
		return
	case "ifMempoolNotEmpty":
		ok, err := lib.IfMempoolNotEmpty(calld.Params, calld.Event)
		if err != nil {
			raftp2p.Revert(err.Error())
			return
		}
		...
		wasmx.Finish(res)
		return
	case "ifOldTransaction":
		ok, err := lib.IfOldTransaction(calld.Params, calld.Event)
		if err != nil {
			raftp2p.Revert(err.Error())
			return
		}
		...
		return
	case "ifMempoolEmpty":
		ok, err := lib.IfMempoolEmpty(calld.Params, calld.Event)
		if err != nil {
			raftp2p.Revert(err.Error())
			return
		}
		...
		return
	case "ifMempoolFull":
		ok, err := lib.IfMempoolFull(calld.Params, calld.Event)
		if err != nil {
			raftp2p.Revert(err.Error())
			return
		}
	...
	case "registerWithKayros":
		_, err := kayrosp2p.RegisterWithKayros(calld.Params, calld.Event)
		if err != nil {
			raftp2p.Revert(err.Error())
			return
		}
	case "getKayrosTxs":
		err := kayrosp2p.GetKayrosTxs(calld.Params, calld.Event)
		if err != nil {
			raftp2p.Revert(err.Error())
			return
		}
	...
	case "receiveCommit":
		if err := kayrosp2p.ReceiveCommit(calld.Params, calld.Event); err != nil {
			raftp2p.Revert(err.Error())
			return
		}
	case "receiveMissingTransactions":
		if err := kayrosp2p.ReceiveMissingTransactions(calld.Params, calld.Event); err != nil {
			raftp2p.Revert(err.Error())
			return
		}
	case "receiveMissingTransactionsRequest":
		if err := kayrosp2p.ReceiveMissingTransactionsRequest(calld.Params, calld.Event); err != nil {
			raftp2p.Revert(err.Error())
			return
		}
	case "ifAllTransactions":
		ok := kayrosp2p.IfAllTransactions(nil, calld.Event)
		res := raft.WrapGuard(ok)
		wasmx.Finish(res)
		return
	...
	case "VerifyCommitLight":
		if err := kayrosp2p.VerifyCommitLight(calld.Params, calld.Event); err != nil {
	...
	case "commitBlock":
		if err := kayrosp2p.CommitBlock(calld.Params, calld.Event); err != nil {
			raftp2p.Revert("commitBlock failed: " + err.Error())
			return
		}
	case "sendCommit":
		if err := kayrosp2p.SendCommit(calld.Params, calld.Event); err != nil {
	...
	case "cancelActiveIntervals":
		...
		kayrosp2p.LoggerDebug("cancelActiveIntervals called", nil)
	case "requestValidatorNodeInfoIfSynced":
		// Request validator node info after state sync - stub for now
		kayrosp2p.LoggerDebug("requestValidatorNodeInfoIfSynced called", nil)
